...
def sanitize_beam_name(beam_name: str) -> str:
...
def extract_and_generate_aperture_data_g1(ds, rt_plan_data: dict, output_base_dir: str):
    ...
    Files are saved in the rtplan folder only.
    ...
    # Create output directory under plan only
    ...
    for i, beam_ds in enumerate(ds.IonBeamSequence):
        ...
        sanitized_beam_name = sanitize_beam_name(beam_name)
        ...
        # Create field directory under plan only
        plan_field_dir = plan_output_dir / sanitized_beam_name
        ...
        if hasattr(beam_ds, 'IonBlockSequence') and beam_ds.IonBlockSequence:
            ...
            if aperture_data:
                # Generate aperture file in the plan folder
                plan_aperture_path = plan_field_dir / f"{sanitized_beam_name.lower()}_aperture.csv"
                
                generate_aperture_csv(beam_name, aperture_data, str(plan_aperture_path))
                
                aperture_files_created.append(str(plan_aperture_path))
                
                print(f"Generated aperture file for G1 beam: {plan_aperture_path}")
        ...
        if _has_mlc_beyond_home_position(beam_ds):
            ...
            if mlc_data:
                # Generate MLC file in the plan folder
                plan_mlc_path = plan_field_dir / f"{sanitized_beam_name.lower()}_mlc.csv"
                
                generate_mlc_csv(beam_name, mlc_data, str(plan_mlc_path))
                
                aperture_files_created.append(str(plan_mlc_path))
                
                print(f"Generated MLC file for G1 beam: {plan_mlc_path}")
